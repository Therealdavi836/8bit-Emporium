...
<html lang="en">
    <head>
        <title>8-bit Emporium |Compras registradas</title>
        <meta charset="utf-8">
    ...
    <body>
        ...
        <section id="register" class="container-grid padding-large position-relative overflow-hidden">
            <div class="container">
                <div class="container mt-5">
                    <h1 class=" text-center text-uppercase mb-4 text-black">Registro de compras con detalle</h1>
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <form action="{{ route('sale_details.store')}}" method="post">
                                ...
                                <div class="d-flex flex-column align-items-center">
                                    <label for="users_id" class="text-center form-label">Id del usuario
                                        <select class="form-control" id="users_id" name="users_id" required>
                                                <option value="users_id">Seleccione un usuario
                                ...
                                <div class="d-flex flex-column align-items-center">
                                    <label for="videogames_id" class="text-center form-label">Id del videojuego
                                        <select class="form-control" id="videogames_id" name="videogames_id" required>
                                                <option value="videogames_id">Seleccione un videojuego
                                                    @foreach ($videogames as $videogame)
                                                        <option value="{{ $videogame->id }}">{{ $videogame->id }} {{ $videogame->title }}</option>
                                                    @endforeach
                                                </option>
                                        </select>
                                    </label>
                                ...
                                <div class="d-flex flex-column align-items-center">
                                    <label for="quantity" class="text-center form-label">Cantidad</label>
                                    <input type="number" name="quantity" id="quantity" class="form-control" required placeholder="Ingrese una cantidad">
                                </div>

                                <div class="d-flex flex-column align-items-center">
                                    <label for="sale_date" class="text-center form-label">Fecha de venta</label>
                                    <input type="date" name="sale_date" id="sale_date" class="form-control" required placeholder="Ingrese una fecha">
                                </div>
                                ...
                                <div class="d-flex justify-content-center mt-4">
                                    <a href="{{ route('report.sale.pdf') }}" class="btn btn-danger me-2">Exportar PDF</a>
                                    <a href="{{ route('report.sale.excel') }}" class="btn btn-success">Exportar Excel</a>
                                </div>
                            </form>
                        </div>
                    </div>
                        ...
                        <table class="table table-striped table-hover" style="border-radius: 10px; overflow: hidden;">
                            ...
                            <tbody>
                                @foreach ($sale_details as $sale_detail)
                                    <tr>
                                    <td style="text-align: center;">{{ $sale_detail->id }}</td>
                                    <td style="text-align: center;">{{ $sale_detail->users_id }}-{{ $sale_detail->user->name }}</td>
                                    <td style="text-align: center;">{{ $sale_detail->videogames_id }}-{{ $sale_detail->videogame->title }}</td>
                                        <td style="text-align: center;">{{ $sale_detail->quantity }}</td>
                                        <td style="text-align: center;">{{ $sale_detail->sale_date }}</td>
                                        <td style="text-align: center;">
                                            <div class="d-flex align-items-center justify-content-center">
                                                <form action="{{ route('sale_details.edit', $sale_detail->id) }}" method="GET" class="me-2">
        ...
        <script src="{{ asset('js/SaleDetailsValidation.js') }}"></script>
