...
<html lang="en">
    <head>
        <title>8-bit Emporium | Detalle de categoría</title>
        <meta charset="utf-8">
    ...
    <body>
        ...
        <section id="register" class="container-grid padding-large position-relative overflow-hidden">
            <div class="container">
                <div class="container mt-5">
                    <h1 class=" text-center text-uppercase mb-4 text-black">Registro de detalle de la categoria</h1>
                    <div class="card mb-4">
                        <div class=" text-center card-header bg-primary text-white">
                            <h4>Agregar detalle de la categoria</h4>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('category_details.store')}}" method="post">
                                @csrf
                                ...
                                <div class="d-flex flex-column align-items-center">
                                    <label name ='categories_id' class="text-center form-label">Seleccione la categoria
                                        <select class="form-control" id="categories_id" name="categories_id" required>
                                                <option value="categories_id">Seleccione una categoria
                                ...
                                <div class="d-flex justify-content-center mt-4">
                                    <a href="{{ route('report.category.pdf') }}" class="btn btn-danger me-2">Exportar PDF</a>
                                    <a href="{{ route('report.category.excel') }}" class="btn btn-success">Exportar Excel</a>
                                </div>
                            </form>
                        </div>
                    </div>
                        ...
                        <table class="table table-striped table-hover" style="border-radius: 10px; overflow: hidden;">
                            ...
                            <tbody>
                                @foreach ($category_details as $category_detail)
                                    <tr>
                                    <td style="text-align: center;">{{ $category_detail->id }}</td>
                                    <td style="text-align: center;">{{ $category_detail->videogames_id }}-{{ $category_detail->videogame->title }}</td>
                                    <td style="text-align: center;">{{ $category_detail->categories_id }}-{{ $category_detail->category->name }}</td>
                                        <td style="text-align: center;">
                                            <div class="d-flex align-items-center justify-content-center">
                                                <form action="{{ route('category_details.edit', $category_detail->id) }}" method="GET" class="me-2">
                                                    <button type="submit" class="btn btn-warning btn-sm">Editar</button>
                                                </form>
                                                <form action="{{ route('category_details.destroy', $category_detail->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
        ...
        <script src="{{ asset('js/CategoryDetailFormValidation.js') }}"></script>
