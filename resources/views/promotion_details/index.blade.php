...
<html lang="en">
    <head>
        <title>8-bit Emporium |Detalle de promoción</title>
        <meta charset="utf-8">
    ...
    <body>
        ...
        <section id="register" class="container-grid padding-large position-relative overflow-hidden">
            <div class="container">
                <div class="container mt-5">
                    <h1 class=" text-center text-uppercase mb-4 text-black">Registro de detalle de las promociones</h1>
                    <div class="card mb-4">
                        <div class=" text-center card-header bg-primary text-white">
                            <h4>Agregar detalle de la promocion</h4>
                        </div>
                        <div class="card-body">
                            <form action="{{ route('promotion_details.store')}}" method="post">
                                @csrf
                                <div class="d-flex flex-column align-items-center">
                                    <label for="videogame_id" class="text-center form-label">Nombre del videojuego
                                        <select class="form-control" id="videogame_id" name="videogame_id" required>
                                ...
                                <div class="d-flex flex-column align-items-center">
                                    <label for="promotion_id" class="text-center form-label">Nombre de la promoción
                                        <select class="form-control" id="promotion_id" name="promotion_id" required>
                                            <option value="">Seleccione una promoción</option>
                                            @foreach ($promotions as $promotion)
                                                <option value="{{ $promotion->id }}">{{ $promotion->id }} {{ $promotion->name }}</option>
                                            @endforeach
                                        </select>
                                    </label>
                                </div>
                                ...
                                <div class="d-flex justify-content-center mt-4">
                                    <a href="{{ route('report.promotion_details.pdf') }}" class="btn btn-danger me-2">Exportar PDF</a>
                                    <a href="{{ route('report.promotion_details.excel') }}" class="btn btn-success">Exportar Excel</a>
                                </div>
                            </form>
                        </div>
                    </div>
                        ...
                        <table class="table table-striped table-hover" style="border-radius: 10px; overflow: hidden;">
                            <thead class="table-bright text-center">
                                <tr>
                                    ...
                                    <th>Promocion del videojuego</th>
                                    <th>Acciones</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach ($promotion_details as $promotion_detail)
                                    <tr>
                                    <td style="text-align: center;">{{ $promotion_detail->id }}</td>
                                    <td style="text-align: center;">{{ $promotion_detail->videogame->title }}</td>
                                    <td style="text-align: center;">{{ $promotion_detail->promotion->name }}</td>
                                        <td style="text-align: center;">
                                            <div class="d-flex align-items-center justify-content-center">
                                                <form action="{{ route('promotion_details.edit', $promotion_detail->id) }}" method="GET" class="me-2">
                                                    <button type="submit" class="btn btn-warning btn-sm">Editar</button>
                                                </form>
                                                <form action="{{ route('promotion_details.destroy', $promotion_detail->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
        ...
        <script src="{{ asset('js/PromotionDetailFormValidation.js') }}"></script>
